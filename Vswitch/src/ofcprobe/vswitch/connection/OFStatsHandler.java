...
import ofcprobe.vswitch.connection.flowtable.OFFlowTable;
import ofcprobe.vswitch.connection.flowtable.OFFlowTableEntry;
...
//			OFFlowStatisticsRequest flowReq = new OFFlowStatisticsRequest();
//			ByteBuffer flowBuf = ByteBuffer.allocate(incomingStat.getLengthU());
//			incomingStat.writeTo(flowBuf);
//			flowBuf.flip();
//			flowReq.readFrom(flowBuf);
		...
		case TABLE:
			...
			break;
		case VENDOR:
			logger.warn("[Switch#{}]: NOT FULLY IMPLEMENTED STAT_REQUEST: {}", this.dpid, incomingStatType.toString());
...
//		stat_reply.setStatisticsFactory()
//		logger.trace("OFStat-Answer: {}", stat_reply.toString());
		return stat_reply;
...
//				}
	...
	private List<OFTableStatistics> tableStatBuilder() {
		List<OFTableStatistics> tableStats = new ArrayList<OFTableStatistics>();
	...
	private List<OFPortStatisticsReply> portReplyBuilder(OFPortStatisticsRequest portRequest) {
		...
		List<OFPortStatisticsReply> replyList = new ArrayList<OFPortStatisticsReply>();
	...
	private OFDescriptionStatistics descStatBuilder(OFStatisticsRequest incomingStat) {
		...
		descStat.setSoftwareDescription("1.0");
